    ...
    test('Solve a puzzle with valid puzzle string: POST request to /api/solve', function (done) {
      ...
      test('Solve a puzzle with invalid characters: POST request to /api/solve', function (done) {
          chai
              ...
              .keepOpen()
              .post('/api/solve')
              .send({puzzle: '..9a.5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..'})
              .end(function (err, res) {
                assert.equal(res.status, 200);
      ...
      test('Check a puzzle placement with all fields: POST request to /api/check', function (done) {
          chai
              ...
              .end(function (err, res) {
                assert.equal(res.status, 200);
                assert.deepEqual(res.body, {"conflict": [], "valid": true});
                done();
              });
      });
      ...
      test('Check a puzzle placement with single placement conflict: POST request to /api/check', function (done) {
          ...
          .end(function (err, res) {
            assert.equal(res.status, 200);
            assert.deepEqual(res.body, {"conflict": ["column"], "valid": false});
            done();
          });
      });
      ...
      test('Check a puzzle placement with multiple placement conflicts: POST request to /api/check', function (done) {
          ...
          .end(function (err, res) {
            assert.equal(res.status, 200);
            assert.deepEqual(res.body, {"conflict": ["column", "row"], "valid": false});
            done();
          });
      });
      ...
      test('Check a puzzle placement with all placement conflicts: POST request to /api/check', function (done) {
          ...
          .end(function (err, res) {
            assert.equal(res.status, 200);
            assert.deepEqual(res.body, {"conflict": ["column", "region", "row"], "valid": false});
            done();
          });
      });
